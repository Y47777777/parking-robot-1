...
{
    ros::init(argc, argv, "laser");       //初始化节点
    ros::NodeHandle n;   
...
void laser_location(float *center_x,float *center_theta,float *center_dis,float *side_dis,float *side_theta)
{
    ...
    else
    {
        ...
        if(fabs(fabs(atan(k_line[1])) - fabs(atan(k_line[2]))) > 20*3.14/180
        || fabs(fabs(atan(1/k_line[0])) - fabs(atan(1/k_line[3]))) > 20*3.14/180){
...
{
    float dis_limit_percent=0.02,angle_max_limit=85*3.1415/180,angle_min_limit=20*3.1415/180;
    int data_num=data_cur->data_num;
    ...
    for(int i=0;i<m;i++){
        ...
        if(dis_aver[i]>0.5 && dis_aver[i]<2){
...
void lidar_calibration(float *dis_,float *theta_)
{
    ...
    close(pfile);
    //--找到校准平面
    plane_find_by_dbscan(angle,dis,plane_angle,plane_dis,num);
    ...
    *dis_ = fabs(B/sqrt(pow(A,2)+1));
    ROS_INFO("plane_theta = %f plane_dis = %f",*theta_*180/3.1415,*dis_);
    return;
}


void plane_find_by_dbscan(float *angle,float *dis,float plane_angle[200],float plane_dis[200],int num[2])
{
    ...
    for(int i=0;i<m;i++){
        // ROS_INFO("i=%d angle_aver=%f dis_aver=%f len=%d mark=%d",i,angle_aver[i]*180/3.1415,dis_aver[i],cluster_len[i],mark[i]);
        if(dis_aver[i]>1 && dis_aver[i]<3){
            if(fabs(angle_aver[i])<angle_max_limit){
                memcpy(plane_angle,angle_cluster[line[i]],200*sizeof(float));
                memcpy(plane_dis,dis_cluster[line[i]],200*sizeof(float));
                num[0]=(int)cluster_len[line[i]];
                num[1]=mark[line[i]];
